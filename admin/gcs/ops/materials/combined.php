...
if (isset($_GET['id_member']) && is_numeric($_GET['id_member'])) {
  ...
  $sql = <<< EOD
select id_gm as id_member from ucon_event where id_convention=$year and b_approval=1
union
select id_member from ucon_order where id_convention=$year
EOD;
  $rs = $db->Execute($sql);
  $ids = array();
  foreach ($rs as $row) {
...
$sql = <<< EOD
  select O.id_member, O.id_order, O.s_type, O.s_special, O.i_price, O.i_quantity, O.s_subtype
    ,E.id_event,E.s_number,trim(E.s_title) as s_title,trim(E.s_game)as s_game,E.i_time,E.e_day
  from ucon_order as O left join ucon_event as E on (O.s_subtype=E.id_event and O.id_convention=E.id_convention)
  where O.id_convention=$year
    and (id_member in ($idString))
  order by id_member, (i_price<0), s_type
EOD;
...
EOD;
$rs = $db->Execute($sql);
//if (!is_array($rs)) die ("SQL Error: ".$db->ErrorMsg());
foreach ($rs as $row) {
...
foreach ($rs as $row) {
  $rowMemberId = $row['id_member'];
  $members[$rowMemberId]->gmrefund = $row['refund'];
}
...
foreach ($rs as $row) {
  $rowMemberId = $row['id_member'];
  $members[$rowMemberId]->volunteerRefund = $row['refund'];
}
...
foreach ($members as $member) {
  ...
  $memberId = $member->row['id_member'];
  $memberName .= " (#$memberId)";
  $balance = $member->total;

  $pdf->SetFont('helvetica','B', 11);
  $pdf->SetFillColor(191,191,191);
  $pdf->Cell($cellWidth-$dpi*1.5, 12, $memberName, $border, 0, 'L', 1, '', 0);
  ...
  $pdf->Cell($dpi*1.5, 12, 'Balance: $'.number_format($balance,2), $border, 1, 'R', 1, '', 0);

  $pdf->SetFont('helvetica','', 9);

  if (count($member->events)>0) {
    $pdf->SetFillColor(191,191,191);
    $pdf->Cell($cellWidth, 10, 'GM Events', $border, 1, 'C', 1, '', 0);

    $pdf->SetFillColor(255, 255, 255);
    foreach ($member->events as $event) {
      $name = $event['s_game'];
      if ($event['s_title']!=$event['s_game']) $name .= ': '.$event['s_title'];
      ...
      $pdf->Cell($dpi*.8, 10, '#'.$event['s_number'], $border, 0, 'C', 1, '', 0);
      $pdf->Cell($dpi*3.5, 10, $name, $border, 0, 'L', 1, '', 0);
      $pdf->Cell($dpi, 10, $time, $border, 0, 'L', 1, '', 0);
  ...
  foreach ($currItems as $item) {
    $count++;
    $type = $item['s_type'];
    $line = trim($item['s_subtype']);
    $line = str_replace(' (comped)','',$line);
    ...
    $pdf->Cell($dpi*.2, 10, $count, $border, 0, 'C', 1, '', 0);
    $pdf->Cell($dpi*.6, 10, $type, $border, 0, 'L', 1, '', 0);

    if ($item['s_type']=='Shirt' || ($item['s_type']=='Misc' && $item['s_subtype']!='Generic Ticket')) {
      $pdf->SetFillColor(255, 255, 0);
      $pdf->SetFont('helvetica','B',9);
    }
    $pdf->Cell($dpi*3.5, 10, $line, $border, 0, 'L', 1, '', 0);

    $pdf->SetTextColor(127,127,127);
    $pdf->SetFont('helvetica','',9);
    $pdf->SetFillColor(255, 255, 255);
    $pdf->Cell($dpi*.6, 10, $itemPrice, $border, 0, 'R', 1, '', 0);
    ...
    $pdf->Cell($dpi*.3, 10, 'x'.$quantity, $border, 0, 'R', 1, '', 0);

    $pdf->SetFont('helvetica','',9);
    $pdf->SetTextColor(0,0,0);
    $pdf->SetFillColor(255, 255, 255);
    $pdf->Cell($dpi*.6, 10, $itemTotal, $border, 
...
//    exit;
  ...
  $pdf->Cell($dpi*.6, 12, '$'.number_format($balance,2), $border, 1, 'R', 1, '', 0);
  ...
  if (isset($member->gmrefund) && $member->gmrefund > 0) {
//die('refund: '.$member->gmrefund);
  ...
  if (isset($member->volunteerRefund) && $member->volunteerRefund > 0) {
//die('refund: '.$member->gmrefund);
    $refund = number_format($member->volunteerRefund,2);
    $pdf->SetFont('helvetica','', 8);
    $pdf->Cell($signatureWidth, 15, $signatureLine, $noborder, 1, 'L', 1, '', 0);
    $pdf->SetFont('helvetica','', 6);
    $pdf->Cell($signatureWidth, 6, " Volunteer Refund Recieved (\${$refund})", $noborder, 1, 'L', 1, '', 0);
  }
}

$pdf->Output('PreregCards.pdf', 'I');
