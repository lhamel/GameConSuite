...
{if $event.buy.icon|default:false}
  ...
  {/if}
{/if}
{if $actions.detail|default:false}<a href="{$actions.detail}{$event.id_event}">{/if}
{$event.s_number}{if $actions.detail|default:false}</a>{/if} <strong>{$event.s_game}{if $event.s_title and $event.s_title!=$event.s_game}: {$event.s_title}{/if}</strong>, GM: {if $actions.navigateMember|default:false}<a href="{$actions.navigateMember}{if $event.s_lname}{$event.s_lname}{else}{$event.s_fname}{/if}">{$event.gamemaster|trim}</a>{else}{$event.gamemaster|trim}{/if}, 
